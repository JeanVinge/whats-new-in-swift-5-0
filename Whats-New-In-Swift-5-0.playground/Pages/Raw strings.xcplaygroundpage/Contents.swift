...
/*:
 Os `#` do inicio e ao final tornam-se parte de delimitadores da string, então o Swift entende que as marcas de citação independentes em torno de “chuva” e “Espanha” deveriam ser tratadas como marcas de citações literais em vez de terminador da string.

 Raw strings aceita usar barras invertidas também:
*/
...
/*:
 ...
 Um dos recursos interessantes de raw strings do Swift é o uso de símbolos de hash no início e no final, pois você pode usar mais de um no caso improvável de ser necessário. É difícil fornecer um bom exemplo aqui, porque realmente deveria ser extremamente raro, mas considere esta sequência: ** Meu cachorro disse "woof" #gooddog **. Como não há espaço antes do hash, o Swift verá `" # `e imediatamente o interpretará como o terminador de string. Nesta situação, precisamos alterar nosso delimitador de `#"` para `##"`, desta forma:
*/
...
/*:
 ...
     As strings raw são totalmente compatíveis com o sistema de strings multi-line - apenas use `#" "` `para iniciar, depois` "" "#` no final, assim:
*/
    let multiline = #"""
    A resposta da vida,
    do universo,
...
/*:
 ...
 Ser capaz de fazer expressões regulares sem muitas barras invertidas será particularmente útil. Por exemplo, escrever um regex simples para localizar caminhos-chave como `\ Person.name` costumava ficar assim:
*/
